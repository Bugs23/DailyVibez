...
export default function Login() {
    ...
    return (
        <div className="flex flex-col flex-1 justify-center items-center gap-4">
            ...
            <p className="text-center">{isRegister ? "Already have an account?" : "Don't have an account?"}</p>
            <div className="max-w-[400px] w-full mx-auto">
                <Button clickHandler={handleSubmit} text={authenticating ? "Submiting" : "Submit"} full />
            </div>
            <button onClick={() => setIsRegister(!isRegister)}>Don't have an account? <span className="text-indigo-600">{isRegister ? "Sign in" : "Sign up"}</span></button>
        </div>
    )
}